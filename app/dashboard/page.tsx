...
import { ExternalLink, CheckCircle2, AlertCircle, Plus, BarChart3, ShoppingBag, Store, Search } from "lucide-react";
...
export default async function DashboardPage({
  ...
  // Fetch connected Google ad accounts for this user
  const { data: googleAccounts } = await supabase
    .from("google_ad_accounts")
    .select("id, google_customer_id, google_account_name, client_id, is_active, connected_at")
    .eq("user_id", user!.id)
    .eq("is_active", true)
    .order("connected_at", { ascending: false });
  ...
  const hasConnectedGoogle = googleAccounts && googleAccounts.length > 0;
    ...
    <div className="space-y-6">
      ...
      {params.google_connected === "true" && (
        <div className="flex items-center gap-2 rounded-lg border border-green-200 bg-green-50 p-4 text-green-800 dark:border-green-800 dark:bg-green-950 dark:text-green-200">
          <CheckCircle2 className="h-5 w-5 shrink-0" />
          <p className="text-sm">
            Successfully connected Google Ad Account
            {params.google_account_name ? `: ${params.google_account_name}` : ""}
          </p>
        </div>
      )}
      ...
      {params.google_error && (
        <div className="flex items-center gap-2 rounded-lg border border-red-200 bg-red-50 p-4 text-red-800 dark:border-red-800 dark:bg-red-950 dark:text-red-200">
          <AlertCircle className="h-5 w-5 shrink-0" />
          <p className="text-sm">
            {params.message ?? "Failed to connect Google Ad Account"}
          </p>
        </div>
      )}
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-semibold">Google Ad Accounts</h3>
          {!hasConnectedGoogle && (
            <a href={`/api/google/connect?client_id=${DEMO_CLIENT_ID}`}>
              <Button>
                <Plus className="mr-1 h-4 w-4" />
                Connect Google Ads
              </Button>
        ...
        {hasConnectedGoogle ? (
              ...
              <Card key={account.id}>
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between">
                    <CardTitle className="text-base flex items-center gap-2">
                      ...
                      {account.google_account_name ?? "Unnamed Account"}
                    </CardTitle>
                    <Badge variant="default" className="bg-green-600">
                      Connected
                    </Badge>
                ...
                <CardContent>
                  ...
                  <div className="mt-3 flex justify-end gap-2">
                    ...
                    <GoogleDisconnectButton clientId={account.client_id} />
                  </div>
                </CardContent>
              </Card>
            ))}
          ...
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <h4 className="text-lg font-semibold">No Google Ads accounts connected</h4>
              <p className="mb-6 max-w-sm text-sm text-muted-foreground">
                Connect your Google Ads account to start tracking campaign
                performance across Search, Display, Shopping, and YouTube.
              </p>
              <a href={`/api/google/connect?client_id=${DEMO_CLIENT_ID}`}>
                <Button size="lg">
                  <Plus className="mr-2 h-4 w-4" />
