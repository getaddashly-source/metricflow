...
export function GoogleSeedDemoButton() {
  ...
  async function handleSeed() {
    ...
    setResult(null);
    try {
      const res = await fetch("/api/google/seed-demo", { method: "POST" });
      const body = await res.json();
      if (!res.ok) {
        setResult(`Error: ${body.error ?? "Failed to seed data"}`);
      } else {
        setResult(`Seeded ${body.total_rows} Google rows (${body.campaigns} campaigns × ${body.days} days)`);
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      <Button onClick={handleSeed} disabled={seeding} variant="secondary">
        ...
        {seeding ? "Seeding…" : "Load Google Demo Data"}
      </Button>
    </div>
  );
}
